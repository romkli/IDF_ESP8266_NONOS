...
void ICACHE_FLASH_ATTR rtc_sw_timer_cb( void *arg ) {
	...
	int8_t t_int;
	uint8_t t_fract;
	if( 0 == mkp_read_temp( st_bme280, &t_int, &t_fract ) ) {
		os_sprintf( buf, "BMPE280 - temperatura: %d,%02d C\r\n", t_int, t_fract );
		os_printf( buf );
	}
	...
	if( !mkp_read_pressure( st_bme280, &hp_int, &hp_fract ) ) {
		os_sprintf( buf, "BMPE280 - Ciœnienie: %d,%02d hPA\r\n", hp_int, hp_fract );
		os_printf( buf );
	}
	...
	if( !mkp_read_humidity( st_bme280, &hm_int, &hm_fract ) ) {
		os_sprintf( buf, "BMPE280 - Wilgotnoœæ: %d,%02d %\r\n", hm_int, hm_fract );
		os_printf( buf );
	}
...
//	int8_t t_int;
//	uint8_t t_fract;
//	if( 0 == mkp_read_temp( st_bmp180, &t_int, &t_fract ) ) {
//		os_sprintf( buf, "BMP180 temperatura: %d,%02d C\r\n", t_int, t_fract );
//		os_printf( buf );
//	}
//
//	/* CIŒNIENIE ATMOSFERYCZNE w hPa */
//	uint16_t hp_int;
//	uint8_t hp_fract;
//	if( !mkp_read_pressure( st_bmp180, &hp_int, &hp_fract ) ) {
//		os_sprintf( buf, "BMP180 Ciœnienie: %d,%02d hPA\r\n", hp_int, hp_fract );
//		os_printf( buf );
//	}
...
//	uint8_t hm_int;
//	uint8_t hm_fract;
//	if( !mkp_read_humidity( st_hih6131, &hm_int, &hm_fract ) ) {
//		os_sprintf( buf, "HIH6131 - Wilgotnoœæ: %d,%02d %\r\n", hm_int, hm_fract );
//		os_printf( buf );
//	}
...
void ICACHE_FLASH_ATTR on_user_init_completed( void ) {
	...
	os_printf( "\r\nChecking BMPE280:\r\n" );
	if( i2c_check_slave( BMPE280_ADDR ) ) os_printf( "ACK OK\r\n" );
	else os_printf( "ACK missing!\r\n" );

	os_printf( "\r\nChecking HIH6131:\r\n" );
	if( i2c_check_slave( HIH6131_ADDR ) ) os_printf( "ACK OK\r\n" );
	else os_printf( "ACK missing!\r\n" );


	os_printf( "\r\nChecking BMP180:\r\n" );
	if( i2c_check_slave( BMP180_ADDR ) ) os_printf( "ACK OK\r\n" );
	else os_printf( "ACK missing!\r\n\r\n" );
