...
void ICACHE_FLASH_ATTR parse_udp_data( char *data, uint16_t len ) {

	char buf[128];

	if( !strncasecmp( data, "160mhz", 6 ) ) {
		if( system_update_cpu_freq( 160 ) ) {
			os_sprintf( buf, "\r\n\r\n+CPU CLOCK CHANGE: Taktowanie CPU: 160 MHz\r\n\r\n" );
		}
	}

	if( !strncasecmp( data, "80mhz", 5 ) ) {
		if( system_update_cpu_freq( 80 ) ) {
			os_sprintf( buf, "\r\n\r\n+CPU CLOCK CHANGE: Taktowanie CPU: 80 MHz\r\n\r\n" );
		}
	}

	if( !strncasecmp( data, "getfcpu", 7 ) ) {
		os_sprintf( buf, "\r\n\r\nF_CPU = %d MHz\r\n\r\n", system_get_cpu_freq() );
	}

	if( !strncasecmp( data, "flashid", 7 ) ) {
		os_sprintf( buf, "\r\n\r\nFLASH ID: 0x%X \r\n\r\n", spi_flash_get_id() );
	}

	if( !strncasecmp( data, "rst", 3 ) ) {
		system_restart();
	}

	if( !strncasecmp( data, "chipid", 6 ) ) {
		os_sprintf( buf, "\r\n\r\nCHIP ID: 0x%X \r\n\r\n", system_get_chip_id() );
	}
...
void ICACHE_FLASH_ATTR scan_done_cb( void *arg, STATUS status ) {
	...
	if (status == OK) {
		...
		WIFI_Connect( WIFI_SSID, "55AA55AA55" );
