...
void ICACHE_FLASH_ATTR at_setupCmdUOTA( uint8_t id ) {

	char buf[128];

	os_sprintf( buf, "FUPDATEME" );
	srv_udp_send( buf, os_strlen( buf ) );

}


void ICACHE_FLASH_ATTR at_setupCmdFota(uint8_t id, char *pPara) {

	char buf[512];

	pPara++; // skip '='

	at_data_str_copy(buf, &pPara, 512);

	at_port_print( "MK FOTA: URL [" );
	at_port_print( buf );
	at_port_print( "]\r\n" );

	at_response_ok();

	mk_start_http_fota( buf );
}
...
//	{"+TEST", 		5, at_testCmdTest, 		at_queryCmdTest, 			at_setupCmdTest, 			at_exeCmdTest		},
	...
	{"+CIUPDATE", 	9, NULL,            	NULL,            			NULL, 						at_exeCmdCiupdate	},	// 4
	{"+FOTA", 		5, NULL,				NULL, 						at_setupCmdFota, 			NULL				},	// 5
	{"+UOTA", 		5, NULL,				NULL, 						NULL, 						at_setupCmdUOTA		},	// 6
};
